...
begin
  ...
  if (cdsDados.FieldByName('id').AsInteger <> 0) then begin
    lblIR.Caption   := FormatFloat('#,##0.00', IFuncController.GetIR(cdsDados.FieldByName('id').AsInteger));
    lblINSS.Caption := FormatFloat('#,##0.00', IFuncController.GetINSS(cdsDados.FieldByName('id').AsInteger));
  end;
end;
